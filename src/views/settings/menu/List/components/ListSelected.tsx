...
const MenuListSelected = () => {
    const { selected, menuList, mutate, total, setAll } = useMenuList()
    ...
    const handleConfirmDelete = useCallback(() => {
        ...
        const remainingMenu = menuList.filter(
            (menu) =>
                !selected.some((sel) => String(sel.id) === String(menu.id)),
        )
    ...
    }, [menuList, selected, total, setAll, mutate])
            ...
            >
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-lg font-semibold text-primary">
                        <TbChecks />
                        {selected.length}{' '}
                        {selected.length > 1 ? 'Menu' : 'Menu'} selected
                    </span>
