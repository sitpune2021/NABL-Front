...
const PrefixListSelected = () => {
    const { selected, prefixList, mutate, total, setAll } = usePrefixList()
    ...
    const handleConfirmDelete = useCallback(() => {
        ...
        const remainingCategories = prefixList.filter(
            (prefix) => !selected.some((sel) => sel.id === prefix.id),
        )
        setAll([])
        mutate(
            {
    ...
    }, [prefixList, selected, total, setAll, mutate])
            ...
            >
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-lg font-semibold text-primary">
                        <TbChecks />
                        {selected.length}{' '}
                        {selected.length > 1 ? 'Categories' : 'Prefix'} selected
                    </span>
...
export default PrefixListSelected
