...
const LabListSelected = () => {
    const { selectedLab, labList, mutate, labListTotal, setSelectAllLab } =
        useLabList()
    ...
    const handleConfirmDelete = () => {
        const newLabList = labList.filter((lab) => {
            return !selectedLab.some((selected) => selected.id === lab.id)
        })
        setSelectAllLab([])
        mutate(
            {
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {selectedLab.map((lab) => (
                        <Tooltip key={lab.id} title={lab.name}>
                            <Avatar size={30} src={lab.img} alt="" />
                        </Tooltip>
                    ))}
                </Avatar.Group> */}
                <div className="my-4">
...
export default LabListSelected
