...
const LabListSelected = () => {
    const { selected, labList, mutate, total, setAll } = useLabList()
    ...
    const handleConfirmDelete = useCallback(() => {
        ...
        const remainingCategories = labList.filter(
    ...
    }, [labList, selected, total, setAll, mutate])
            ...
            >
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-lg font-semibold text-primary">
                        <TbChecks />
                        {selected.length} {selected.length > 1 ? 'Labs' : 'Lab'}{' '}
                        selected
                    </span>
