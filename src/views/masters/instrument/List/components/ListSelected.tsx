...
const InstrumentListSelected = () => {
    const { selected, instrumentList, mutate, total, setAll } =
        useInstrumentList()
    ...
    const handleConfirmDelete = useCallback(() => {
        ...
        const remainingCategories = instrumentList.filter(
            (instrument) => !selected.some((sel) => sel.id === instrument.id),
        )
        setAll([])
        mutate(
            {
    ...
    }, [instrumentList, selected, total, setAll, mutate])
            ...
            >
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-lg font-semibold text-primary">
                        <TbChecks />
                        {selected.length}{' '}
                        {selected.length > 1 ? 'Categories' : 'Instrument'}{' '}
                        selected
                    </span>
...
export default InstrumentListSelected
