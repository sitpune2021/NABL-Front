...
import {
    apiApproveDepartments,
    apiGetPendingDepartments,
} from '@/services/DepartmentService'
import { Department, GetDepartmentListResponse } from '@/@types/department'
import useDepartmentList from '../hooks/useList'

const DepartmentPendingDrawer = () => {
    ...
    const fetchDepartments = useCallback(async () => {
        try {
            ...
            const res =
                (await apiGetPendingDepartments()) as GetDepartmentListResponse
            const data = res?.data ?? []
    ...
    const handleApprove = useCallback(async () => {
        ...
        try {
            setLoading(true)
            await apiApproveDepartments(selectedIds)
            mutate()
    ...
    }, [selectedIds, mutate, fetchDepartments])

    const allIds = useMemo(
        () => departments.map((c) => Number(c.id)),
        [departments],
    )
                    ...
                    <>
                        <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-100  top-0 z-10">
                            <span className="text-xs text-gray-500 font-medium uppercase tracking-wide">
                                {selectedIds.length} of {departments.length}{' '}
                                Selected
                            </span>
                            <Checkbox
                ...
                {!loading && !departments.length && (
                    <div className="flex flex-col items-center justify-center py-24 text-center px-6">
                        ...
                        <h4 className="text-lg font-semibold text-gray-900">
                            No Pending Departments
                        </h4>
                        <p className="text-gray-500 text-sm mt-1">
                            All departments are already approved.
                        </p>
                    </div>
                )}
            </Drawer>
...
export default DepartmentPendingDrawer
