...
const DepartmentListSelected = () => {
    const { selected, departmentList, mutate, total, setAll } =
        useDepartmentList()
    ...
    const handleConfirmDelete = useCallback(() => {
        ...
        const remainingdepartments = departmentList.filter(
            (department) => !selected.some((sel) => sel.id === department.id),
        )
        setAll([])
        mutate(
            {
                data: remainingdepartments,
                total: total - selected.length,
            },
            false,
        )
        setIsDeleteOpen(false)
    }, [departmentList, selected, total, setAll, mutate])
