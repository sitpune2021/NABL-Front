...
const ZoneListSelected = () => {
    ...
    const handleConfirmDelete = useCallback(() => {
        ...
        const remainingCategories = zoneList.filter(
            (zone) => !selected.some((sel) => sel.id === zone.id),
        )
        setAll([])
        mutate(
            {
    ...
    }, [zoneList, selected, total, setAll, mutate])
            ...
            >
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-lg font-semibold text-primary">
                        <TbChecks />
                        {selected.length}{' '}
                        {selected.length > 1 ? 'Categories' : 'Zone'} selected
                    </span>
