...
const ZoneListSelected = () => {
    const { selectedZone, zoneList, mutate, zoneListTotal, setSelectAllZone } =
        useZoneList()
    ...
    const handleConfirmDelete = () => {
        const newZoneList = zoneList.filter((zone) => {
            return !selectedZone.some((selected) => selected.id === zone.id)
        })
        setSelectAllZone([])
        mutate(
            {
                list: newZoneList,
                total: zoneListTotal - selectedZone.length,
            },
            false,
        )
        setDeleteConfirmationOpen(false)
            ...
            >
                ...
                >
                    {selectedZone.map((zone) => (
                        <Tooltip key={zone.id} title={zone.name}>
                            <Avatar size={30} src={zone.img} alt="" />
                        </Tooltip>
                    ))}
                </Avatar.Group> */}
                <div className="my-4">
...
export default ZoneListSelected
