...
const UnitListSelected = () => {
    const { selectedUnit, unitList, mutate, unitListTotal, setSelectAllUnit } =
        useUnitList()
    ...
    const handleConfirmDelete = () => {
        const newUnitList = unitList.filter((unit) => {
            return !selectedUnit.some((selected) => selected.id === unit.id)
        })
        setSelectAllUnit([])
        mutate(
            {
                list: newUnitList,
                total: unitListTotal - selectedUnit.length,
            },
            false,
        )
        setDeleteConfirmationOpen(false)
                ...
                >
                    {selectedUnit.map((unit) => (
                        <Tooltip key={unit.id} title={unit.name}>
                            <Avatar size={30} src={unit.img} alt="" />
                        </Tooltip>
                    ))}
                </Avatar.Group> */}
                <div className="my-4">
